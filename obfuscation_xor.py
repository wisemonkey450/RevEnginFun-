...
def gen_decode_func(fd, cmd):
    ...
    fd.write("\t\tdecoded[i] = %s[i] ^ %s[i];\n" % (cmd.split(" ")[0]+ "_1", cmd.split(" ")[0] + "_2"))
    fd.write("\t}\n")
...
def gen_xor(fd, filename, cmd_comma_list):
    ...
    for cmd in cmd_list:
        ...
        fd.write("int %s_1[%d] = " % (cmd.split(" ")[0], len(cmd)))

        count = 0
        fd.write("{")
        for val in enc:
            fd.write(hex(ord(val)))
            if count != len(enc)-1:
                fd.write(",")
            count = count + 1
        ...
        fd.write("int %s_2[%d] = " % (cmd.split(" ")[0], len(cmd)))
        count = 0
        fd.write("{")
        for val in pkey:
            fd.write(hex(ord(val)))
